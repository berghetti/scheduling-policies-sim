...
class Simulation:
    ...
    def run(self):
        ...
        if self.config.afp_enable:
            print('AFP\nP.O: {} P.Q: {} AFP-II: {} AFP_SL: {}'.format(
                self.config.PREEMPTION_OVERHEAD,
                self.config.PREEMPTION_QUANTUM,
                self.config.afp_unconditional_interrupt,
                self.config.AFP_STARVATION_COUNTER_THRESHOLD))
        ...
        while self.state.any_incomplete():# and \
            ...
            while task_number < self.state.tasks_scheduled and \
                ...
                if self.config.join_bounded_shortest_queue:
                    ...
                    self.state.persephone_dispatcher.queue.enqueue(self.state.tasks[task_number], set_original=True)

                elif self.config.afp_enable and self.config.afp_rr:
                    # round robin
                    w_queue = self.state.queues[afp_worker % len(self.state.available_queues)]
                    w_queue.enqueue(self.state.tasks[task_number], set_original=True)
                    afp_worker += 1
        ...
        print('count_interrupts: {}'.format(self.config.count_interrupts))
